...
class BankMuamalat extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $config = Configuration::getMultiple(array('BANK_MUAMALAT_DETAILS', 'BANK_MUAMALAT_OWNER', 'BANK_MUAMALAT_ADDRESS'));
        ...
        $this->displayName = $this->l('Bank Muamalat');
        $this->description = $this->l('Accept payments for your products via Bank Muamalat transfer.');
    ...
    public function install()
    {
        ...
        $newState->module_name = 'bankmuamalat';
        $newState->invoice = false;
        $newState->color = "#8E2B86";
        ...
        foreach ($languages as $language) {
            if ($language['iso_code'] == 'id') {
                $newState->name[(int)$language['id_lang']] = 'Menunggu pembayaran via Bank Muamalat';
            } else {
                $newState->name[(int)$language['id_lang']] = 'Awaiting Bank Muamalat Payment';
            }
        ...
        if ($newState->add()) {
            ...
            foreach ($languages as $language) {
                if ($language['iso_code'] == 'id') {
                    copy(dirname(__FILE__).'/mails/id/bankmuamalat.html', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankmuamalat.html');
                    copy(dirname(__FILE__).'/mails/id/bankmuamalat.txt', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankmuamalat.txt');
                } else {
                    copy(dirname(__FILE__).'/mails/en/bankmuamalat.html', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankmuamalat.html');
                    copy(dirname(__FILE__).'/mails/en/bankmuamalat.txt', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankmuamalat.txt');
                }
            }
        } else {
            return false;
    ...
    {
        if (!Configuration::deleteByName('BANK_MUAMALAT_DETAILS') || !Configuration::deleteByName('BANK_MUAMALAT_OWNER') || !Configuration::deleteByName('BANK_MUAMALAT_ADDRESS') || !parent::uninstall()) {
            return false;
        }
        return true;
    }
    ...
    public function hookDisplayPaymentEU($params)
    {
        ...
        $payment_options = array(
            'cta_text' => $this->l('Pay by Bank Muamalat'),
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (in_array($state, array(Configuration::get('PS_OS_BANKMUAMALAT'), Configuration::get('PS_OS_OUTOFSTOCK'), Configuration::get('PS_OS_OUTOFSTOCK_UNPAID')))) {
            $this->smarty->assign(array(
    ...
    {
        $this->context->controller->addCSS(($this->_path).'bankmuamalat.css', 'all');
    }
}
